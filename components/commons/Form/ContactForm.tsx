...
const Form = () => {
  ...
  async function submitFormHandler(event: React.FormEvent) {
    ...
    const message = {
      ...
      messages: state.messages || "No Messages",
    };
      ...
      <div className='w-full pb-10 bg-hero-bottom dark:bg-hero-dark-bottom'>
        ...
        >
          ...
          <div className='flex flex-col gap-1'>
            ...
            >
              Phone{" "}
              {error.phoneError && "is Not Valid (maybe add your country code)"}
            </label>
            <input
              type='text'
              id='phone'
          ...
          <div className='flex gap-2'>
            <input id='verificationBox' type='checkbox' onChange={checkBoxHandler} checked={isChecked} />
            <label htmlFor='verificationBox'>
              Are you sure you have filled out the form correctly?
            </label>
          </div>
          <button
            type='submit'
            disabled={!isChecked}
            className='disabled:cursor-not-allowed bg-yellow-custom w-full dark:bg-blue-gray p-2 dark:text-sky-900 font-bold rounded-sm mt-2'
          >
            Send to Hadzami
          </button>
        </form>
